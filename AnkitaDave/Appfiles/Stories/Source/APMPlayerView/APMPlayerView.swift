...
//CALL BACK
protocol APMPlayerObserver: class {
...
class APMPlayerView: UIView {
    ...
    var currentTime: Float {
        let value = Float(player?.currentTime().value ?? 0)
        let timescale = Float(player?.currentTime().timescale ?? 0)
        let currentSeconds = value / timescale
        return currentSeconds
    }
    
    //MARK: - Public Vars
    public weak var playerObserverDelegate: APMPlayerObserver?
...
extension APMPlayerView: PlayerControls {
