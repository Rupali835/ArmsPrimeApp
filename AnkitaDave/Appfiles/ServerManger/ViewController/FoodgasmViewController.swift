...
class FoodgasmViewController: BaseViewController,UITableViewDelegate, UITableViewDataSource, MainTableViewCellDelegate, PurchaseContentProtocol {
    ...
    @IBOutlet var foodgasmTableView: UITableView!
    ...
    override func viewDidLoad() {
        ...
        foodgasmTableView.register(UINib(nibName: "MainTableViewCell", bundle: nil), forCellReuseIdentifier: "MainTableViewCell")
        foodgasmTableView.dataSource = self
        foodgasmTableView.delegate = self
        
        self.foodgasmTableView.estimatedRowHeight = 200
        self.foodgasmTableView.rowHeight = UITableView.automaticDimension
        self.foodgasmTableView.setNeedsUpdateConstraints()
...
//        self.navigationController?.navigationBar.titleTextAttributes = [ NSAttributedString.Key.font: UIFont(name: "Montserrat-Regular", size: 20)!,  NSAttributedString.Key.foregroundColor: UIColor.black]
        ...
        self.foodgasmTableView.reloadData()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if BucketValues.bucketIdArray.count > 0 {
            if #available(iOS 10.0, *) {
                foodgasmTableView.refreshControl = refreshControl
            } else {
                foodgasmTableView.addSubview(refreshControl)
            }
    ...
    func didShareButton(_ sender: UIButton) {
        ...
        var params: String = " "
        params += "&bucket_code=" + "foodgasm"//"\(content.bucket_id ?? "")"
    ...
    func contentPurchaseSuccessful(index: Int, contentId: String?) {
        ...
        if let cell : MainTableViewCell = self.foodgasmTableView.cellForRow(at: IndexPath.init(item: index, section: 0)) as? MainTableViewCell{
...
//        self.showToast(message: "Unlocked Successfully")
        ...
        if (Reachability.isConnectedToNetwork() == true) {
            ...
            alert.addAction(UIAlertAction(title: "Ok", style: .default, handler: { (action) in
                if self.isLogin {
                    ...
                    guard let cell =  self.foodgasmTableView.cellForRow(at: indexPath) as? MainTableViewCell else {
                        return
                    }
    ...
    func didLikeButton(_ sender: UIButton) {
        if !self.checkIsUserLoggedIn() {
            self.showAlert(message: Messages.loginAlertMsg)
             CustomMoEngage.shared.sendEventForLike(contentId: self.dataArray[sender.tag]._id ?? "", status: "Failed", reason:"User not logged in", extraParamDict: nil)
            return
        }
        let indexPath = IndexPath(row: sender.tag, section: 0)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = foodgasmTableView.dequeueReusableCell(withIdentifier: "MainTableViewCell", for: indexPath)as!MainTableViewCell
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if indexPath.row == self.dataArray.count - 1 && self.totalPages > pageNumber {
            if ( Reachability.isConnectedToNetwork() == true) {
                ...
                spinner.frame = CGRect(x: CGFloat(0), y: CGFloat(0), width: foodgasmTableView.bounds.width, height: CGFloat(10))
                self.foodgasmTableView.tableFooterView = spinner
                self.foodgasmTableView.tableFooterView?.isHidden = false
                    ...
                    if currentList.type != nil && currentList.type == "video" {
                                ...
                                if let url = dict.object(forKey: "url") as? String {
                                    ...
                                    if (GlobalFunctions.isContentPiadWithDictCoins(dict: dict as! Dictionary<String, Any>)) && currentList.coins != 0{
                                        ...
                                        if (GlobalFunctions.isContentsPurchased(list: currentList)) {
                                            ...
                                            NotificationCenter.default.addObserver(self, selector: #selector(FoodgasmViewController.didfinishplaying(note:)),name:NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: player.currentItem)
                        ...
                        if isVideo!{
                            ...
                            if dict.object(forKey: "is_album") as? Int == 1 || dict.object(forKey: "is_album") as? Bool == true
                            {
                                ...
                                destViewController.codes = "foodgasm"
                                ...
                                SingleImageScrollViewController.selectedBucketName = BucketValues.bucketTitleArr[selectedIndexVal]
                                self.pushAnimation()
    ...
    func getData()
    {
        ...
        if Reachability.isConnectedToNetwork() == true {
            ...
            if let bucketId = self.bucketId {
                ...
                //                let dict = ["bucket_id": bucketId , "page": "\(pageNumber)", "perpage": "5"] as [String: Any] //streamType":"start/end (Depending on the status), "
                //                print(dict)
                //                if let jsonData = try? JSONSerialization.data(withJSONObject: dict, options: .prettyPrinted) {
                    ...
                    if error != nil{
                        ...
                        DispatchQueue.main.async {
                            ...
                            if (self.database != nil) {
                                ...
                                self.dataArray =  self.database.getSocialJunctionFromDatabase(datatype: "BehindTheScenes")
                                if  self.dataArray.count > 0 {
                                    self.internetConnectionLostView.isHidden = true
                                }
                                self.foodgasmTableView.reloadData()
                    ...
                    do {
                        if (data != nil) {
                            ...
                            if json?.object(forKey: "error") as? Bool == true
                            {
                                ...
                                if let event = GAIDictionaryBuilder.createEvent(withCategory: "Home \(self.navigationTittle) Screen ios", action: "API Pagination Number \(self.currentPage) (ios) ", label: "Success", value: nil).build() as? [AnyHashable : Any] {
                                        ...
                                        if  let photoDetailsArr = dictData["list"] as? NSMutableArray{
                                            ...
                                            for dict in photoDetailsArr{
                                                ...
                                                if (!GlobalFunctions.checkContentBlockId(id: list._id!)) {
                                                    ...
                                                    self.database.insertIntoSocialTable(list:list, datatype: "BehindTheScenes")
            ...
            if keyPath == "rate" {
                ...
                if playerViewController.player!.rate > 0 {
                    ...
                    if let currentList = self.dataArray[self.selectedIndexs] as? List {
                        CustomMoEngage.shared.sendEventViewVide(id: currentList._id ?? "", coins: currentList.coins ?? 0, bucketCode:selectedBucketCode ?? "foodgasam", bucketName: navigationTittle, videoName: currentList.caption ?? "", type: currentList.commercial_type ?? "", percent: percentage)
                    }
                }
            }
        }
